/***********************************
ASSIGMENT 5: PROBLEM 2 (OVERLAYGOPHER)
************************************/
...
socklen_t len;
char readbuf[200], sendbuf[200], buf[200], ack[10], port_char[10], ip[15], postport[5];
fd_set readfds;
struct ifreq ifr;
...
//signal handler for new request
void io_handler(){
...
//Handles the request message from the previous router/createoverlay
void request_handler(){
	flag = 0;
...
//processes the requests and sends back an ack
void request_prev(){
	...
	//forwards the request if it is not the last router and waits for an ack
	if(no_hash>3){
	...
	//receive the first message from the client and update the lookup table
...
//communication between mutiple server-client
void communicate(){
	int sd;
	...
	if(TABLEUPDATE==1){
		...
		printf("INDEX     PREVIOUS SOCK         PREVIOUS-IP       POST SOCK      POST-IP");
		for(int i=0;i<req+1;i++){
			printf("\n %d       %d                  %s     %d         %s           \n", t[i].index+1, t[i].pre_port, t[i].pre_ip, t[i].post_port, t[i].post_ip);
			printf("____________________________________________________________________________\n");
		}
	}
	...
	while(1){
		...
		for (int i = 0; i < MAXSOCKIND; i++) {
			...
			printf("\n Previous sock....");
				...
				{   if((i == t[i].index)&&(strcmp(t[i].pre_ip,inet_ntoa(preaddr[i].sin_addr))==0)){
					printf("\n %s", inet_ntoa(preaddr[i].sin_addr));
					printf("\n %s", t[i].pre_ip);
				    //set the string terminating NULL byte on the end of the data read
