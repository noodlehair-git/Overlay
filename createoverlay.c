/***********************************
ASSIGMENT 5: PROBLEM 2 (CREATEOVERLAY)
************************************/
#include <stdio.h>
#include <sys/stat.h>
#include <sys/types.h>
...
int main(int argc, char** argv) {
	...
	int sockfd, k;
	char readbuf[100], sendbuf[200], ip[20], hash[2], k_char[5];
	socklen_t len;
	...
	strcpy(sendbuf, k_char);
		...
		for(int len = strlen(sendbuf);len<200; len++)
			sendbuf[len]='\0';
	...
	sleep(1);
        //waiting for the ack from router
